...
test("a listing below the Fair suggestion is not comparable and never undercuts", () => {
...
test("a comparable listing caps the target at lowest − 1", () => {
  const p = planPrice({ price: 40, suggestion: 40, fairSuggestion: 10, lowest: 30, forSale: 12 });
...
test("a comparable listing below the tier floor is ignored as noise", () => {
  ...
  const p = planPrice({ price: 40, suggestion: 40, fairSuggestion: 10, lowest: 20, forSale: 12 });
  assert.equal(p.lowestPlausible, true);
  assert.equal(p.competitive, null);
...
test("time decay shaves 5% (at least $1) after 30 quiet days, down to the floor", () => {
  ...
  // can't step down, so the market rule (70% → $21) is what comes back
  ...
  assert.equal(atFloor.target, 21);
...
test("without a Fair suggestion the bar is half the grade suggestion", () => {
