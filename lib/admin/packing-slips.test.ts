...
test("manifest fits a full page of boxes on one 4x6 and spills to a second", async () => {
  const many = (n: number) =>
    Array.from({ length: n }, (_, i) => row({ boxId: 100 + i, buyer: `buyer${i}` }));
...
test("manifest survives labeled and unboxed rows, a missing address and a long buyer name", async () => {
  const bytes = await shipManifest([
    row({ boxIndex: 1, boxCount: 2, tracking: "9400111899223" }),
    row({ boxId: 13, boxIndex: 2, boxCount: 2, shipTo: null }),
    row({ boxId: null, buyer: "a_reddit_handle_that_is_really_quite_long_indeed", records: 5 }),
    row({ boxId: 15, shipTo: { ...slip().shipTo!, name: "坂本龍一" } }),
  ]);
  assert.equal((await PDFDocument.load(bytes)).getPageCount(), 1);
});

test("manifest with no packages is an error", async () => {
  await assert.rejects(shipManifest([]), /No packages/);
});
